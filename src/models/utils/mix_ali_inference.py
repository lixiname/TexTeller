...
#from ...cbn.detection_onnx import inference_of_onnx_version, dataload_of_onnx_version
from ...enumUtils import AlgorithmsEnum
...
def ali_inference(
        ...
        num_beams=1,
        rec_ali: bool=True,
        det_type: AlgorithmsEnum=AlgorithmsEnum.V1
) -> str:
    '''
    ...
    if det_type == AlgorithmsEnum.V1:
        ...
        inline_module_json = {
            'sorted_inline': True,
            'merge': False,
            'split_multi_line': False
        }
        test_loader, cfg = dataload_of_onnx_version(input_dir)

        inference_of_onnx_version(ocr_detection_model, input_dir, test_loader, cfg, inline_module_json, queue,
                                  logger_queue)
    ...
    if rec_ali:
        rec_predictions = []
        for idx, crop_img in enumerate(sliced_imgs):
            res_str, char_list = rec_model(crop_img)
            content = [res_str]
            rec_predictions.append(content)
    # ali
    else:
